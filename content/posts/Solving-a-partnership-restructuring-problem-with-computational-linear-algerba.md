...
Consider a partnership restructuring its assets, with N partners sharing each of M assets unevenly. To minimize possible taxes inccured from the restructuring, I will aim to re-distribute the assets under majority ownerships **without** altering each partner's overall value and debt stake in the business. In application, we used a Markov model to exhaustively search for solutions to a three partner/eleven asset problem, testing over 200 million potential restructure states. The optimal solution incurred a $2.9mm total stake change against $159mm in total assets, an acceptable magnitude of change (<2%). 
